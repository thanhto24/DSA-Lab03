...
void selectionSortVerComp(int arr[], int n, long long &count_compare)
{
	for (int i = 0; ++count_compare && i < n - 1; i++)
	{
		...
		for (int j = i + 1; ++count_compare && j < n; j++)
			if (++count_compare && arr[j] < arr[idx])
				idx = j;
		swap(arr[i], arr[idx]);
	}
}
...
void insertionSortVerComp(int arr[], int n, long long &count_compare)
{
	for (int i = 1; ++count_compare && i < n; i++)
	{
		int val = arr[i];
		int j = i - 1;
		while (++count_compare && j >= 0 && ++count_compare && arr[j] > val)
		{
			arr[j + 1] = arr[j];
			j--;
		}
...
void quickSortDoVerComp(int arr[], int first, int last, long long &count_compare)
{
	...
	if (++count_compare && first < j)
		quickSortDoVerComp(arr, first, j, count_compare);
	if (++count_compare && i < last)
		quickSortDoVerComp(arr, i, last, count_compare);
}
...
void quickSortDoVerTime(int arr[], int first, int last)
{
	...
	if (first < j)
		quickSortDoVerTime(arr, first, j);
	if (i < last)
		quickSortDoVerTime(arr, i, last);
}
